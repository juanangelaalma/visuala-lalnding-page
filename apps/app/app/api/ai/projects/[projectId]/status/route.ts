...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const generationDtos = await Promise.all(generations.data.map(generationDto));
    return Response.json({project:{id:project.data.id,status:project.data.status,durationSeconds:project.data.duration_seconds,quality:project.data.quality,createdAt:project.data.created_at,updatedAt:project.data.updated_at},scenes:scenes.data.map(sceneDto),generations:generationDtos,finalComposition:{status:"waiting_for_composer",processed:false}});
