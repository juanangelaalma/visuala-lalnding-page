...
async function api<T>(fetcher: Fetch, input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok)
    throw new Error(body?.error?.message || "The AI request could not be completed");
  return body as T;
}
...
export async function analyzeAndUpload(files: File[], fetcher: Fetch = fetch) {
  const [analysis, upload] = await Promise.all([
    api<{ product: AiProduct }>(fetcher, "/api/ai/analyze", {
      method: "POST",
      body: imageForm(files),
    }),
    api<{ assets: string[] }>(fetcher, "/api/ai/assets", {
      method: "POST",
      body: imageForm(files),
    }),
  ]);
...
export async function createStoryboard(
  ...
  return api<{ project: { id: string }; scenes: AiScene[] }>(
    fetcher,
    "/api/ai/projects/storyboard",
    {
      method: "POST",
      headers: {
...
async function waitForGenerations(
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    const generations = generationIds.map((id) =>
      status.generations.find((item) => item.id === id),
    );
    if (
      generations.some(
        (item) => item && terminalStatuses.has(item.status) && item.status !== "succeeded",
      )
    ) {
      throw new Error("One or more AI generations failed. Please try again.");
    }
    if (generations.every((item) => item?.status === "succeeded"))
      return { status, generations: generations as Generation[] };
    await delay(3_000);
  }
...
export async function generateVideo(
  ...
  await Promise.all(
    scenes.map((scene) =>
      api(fetcher, `/api/ai/scenes/${scene.id}`, {
        method: "PATCH",
  ...
  const imageRequests = await Promise.all(
    scenes.map((scene) =>
      api<{ generation: Generation }>(fetcher, `/api/ai/scenes/${scene.id}/image`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          references: referenceAssets,
          idempotencyKey: `${idempotencyKey}:image:${scene.id}`,
  ...
  onProgress?.(30);
  const images = await waitForGenerations(
    projectId,
    imageRequests.map((item) => item.generation.id),
    fetcher,
    delay,
  );
  onProgress?.(55);
  ...
  await Promise.all(
    images.generations.map((generation) =>
      api(fetcher, `/api/ai/scenes/${generation.sceneId}/approve`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  const videoRequest = await api<{ generations: Generation[] }>(
    fetcher,
    `/api/ai/projects/${projectId}/video`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  const videos = await waitForGenerations(
    projectId,
    remoteVideos.map((generation) => generation.id),
    fetcher,
    delay,
  );
  onProgress?.(100);
  return {
    videoUrl: videos.generations.flatMap((generation) => generation.assets)[0] ?? null,
