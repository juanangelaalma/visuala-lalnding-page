...
describe("AI video frontend/backend feature integration", () => {
  it("persists scenes, generates and approves images, then renders and polls the video", async () => {
    ...
    const fetcher = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/assets"))
        return Response.json(
          {
            assets: ["ai/user-1/references/0f694b76-9e3f-44e9-8c7a-8b57e9bb74b0.png"],
          },
          { status: 201 },
        );
      if (url.endsWith("/storyboard"))
        return Response.json({ project: { id: "project-1" }, scenes: [scene] }, { status: 201 });
        ...
        const generation =
          statusRequest === 1
            ? {
                ...
                type: "image",
                status: "succeeded",
                assets: ["https://assets.test/image.webp"],
                errorCode: null,
              }
            : {
                id: "video-1",
                ...
                type: "video",
                status: "succeeded",
                assets: ["https://assets.test/video.mp4"],
                errorCode: null,
              };
        return Response.json({
          project: { id: "project-1", status: "generating_scenes" },
          scenes: [scene],
    ...
    const analyzed = await analyzeAndUpload(
    ...
    expect(requests[2].body).toMatchObject({
      ...
      creator: "nadia",
      quality: "standard",
      referenceAssets: ["ai/user-1/references/0f694b76-9e3f-44e9-8c7a-8b57e9bb74b0.png"],
    });
    ...
    expect(requests[4].body).toMatchObject({
      references: ["ai/user-1/references/0f694b76-9e3f-44e9-8c7a-8b57e9bb74b0.png"],
      idempotencyKey: "render-request-1:image:scene-1",
      maxEstimatedCostUsd: 0.1,
    });
    expect(requests[6].body).toEqual({ generationId: "image-1" });
    expect(requests[7].body).toEqual({
      idempotencyKey: "render-request-1:video",
